...
__global__ void matrixMulTiledKernel(const float *A, const float *B, float *C, int width) {
    ...
    for (int t = 0; t < numTiles; ++t) {
        if (row < width && t * TILE_DIM + tx < width) ds_A[ty][tx] = A[row * width + t * TILE_DIM + tx];
        else ds_A[ty][tx] = 0.0f;

        if (t * TILE_DIM + ty < width && col < width) ds_B[ty][tx] = B[(t * TILE_DIM + ty) * width + col];
        else ds_B[ty][tx] = 0.0f;

        __syncthreads();

        for (int k = 0; k < TILE_DIM; ++k) {
             if (t * TILE_DIM + k < width) Cvalue += ds_A[ty][k] * ds_B[k][tx];
        }
        __syncthreads(); 
    }
